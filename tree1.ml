...
let val_set t =
  match t with
    ...
    | Node(_, (s_low,s_up), _, _, quantity) -> 
        if_bad (s_up - s_low + 1 + quantity);;

(* make creates a tree from left_tree (l), interval (k) 
   and right_tree (r) 

   Assumptions: l,k and r are balanced
   Time Complexity: O(1)
   Space Completixy: O(1)
...
(*[bal l k r] balances a new set from sets l and r and interval k 
  ...
  Assumptions: l and k are already balanced
  Time Complexity: O(1)
  Space Completixy: O(1)
  Post-Assumptions: a new set is balanced *)
...
let below x t =
  let rec below_pom x t =
    match t with
      ...
      | Node (l, k, r, _, _) ->
          if (if_in x k) = 0 then
            if_bad (val_set l + x - fst k + 1)
          else if (if_in x k) = -1 then
            if_bad (below_pom x l)
          else 
            if_bad (val_set l + snd k - fst k + 1 + below_pom x r)
  in below_pom x t;;
...
let rec left_interval (x, y) t =
  match t with
    ...
    | Node(l, k, r, _, _) ->
        ...
        else if snd k < x then left_interval (x, y) r
        else left_interval (x, y) l;;

(*the same as left interval byt returns the right one 
...
let rec right_interval (x, y) t =
  match t with
    ...
    | Node(l, k, r, _, _) ->
        ...
        else if fst k > y then right_interval (x, y) l
        else right_interval (x, y) r;;
